...
export default function CarritoPage() {
  ...
  // 1) Load user and rawCart from localStorage
  useEffect(() => {
    ...
    if (saved) setRawCart(JSON.parse(saved));
  }, [router]);

  // 2) Whenever rawCart changes, fetch full product details
  useEffect(() => {
    getProductos().then((productos) => {
      const items = rawCart.map((entry) => {
        const prod = productos.find((p) => p.id_producto === entry.id_producto);
        return prod ? { ...prod, cantidad: entry.cantidad } : null;
      }).filter(Boolean) as (Producto & { cantidad: number })[];
      setCartItems(items);
    });
  }, [rawCart]);

  // 3) Update rawCart and persist
  ...
  const updateQuantity = (id: number, newQty: number) => {
    ...
    const updated = rawCart.map((e) =>
      e.id_producto === id ? { ...e, cantidad: newQty } : e
    );
    updateRawCart(updated);
  };

  const removeItem = (id: number) => {
    const filtered = rawCart.filter((e) => e.id_producto !== id);
    updateRawCart(filtered);
  };
              ...
              <Card className="sticky top-4">
                ...
                <CardFooter>
                  <Link href="/cliente/checkout" className="w-full">
                    <Button className="w-full bg-blue-900 hover:bg-blue-800 text-lg py-3">
                      Proceder al Pago
                    </Button>
                  </Link>
                </CardFooter>
